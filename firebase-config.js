...
onAuthStateChanged(auth, (user) => {
  renderAuthUI(user);
  window.DeenpathAuth = window.DeenpathAuth || {};
  window.DeenpathAuth.currentUser = user;
  window.dispatchEvent(new CustomEvent("deenpath-auth-changed", { detail: { user } }));
});

/* ---------- এডমিন চেক (Firestore 'admins' কালেকশনে UID থাকলে এডমিন) ---------- */
...
/* ---------- ব্লগ পোস্ট হেল্পার (Firestore কালেকশন: 'posts') ---------- */
...
async function createPost({ title, category, imageUrl, body, status, authorName, authorUid }) {
...
async function updatePost(postId, fields) {
  return updateDoc(doc(db, "posts", postId), { ...fields, updatedAt: serverTimestamp() });
}
